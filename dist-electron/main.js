...
import require$$1$1, { join, dirname } from "path";
import require$$3 from "http";
import require$$4 from "https";
import require$$0$1 from "url";
import require$$6, { existsSync, mkdirSync, writeFileSync, unlinkSync } from "fs";
...
class HttpClient {
  constructor(tokenStore2, baseURL = "https://api.saywrite.nously.io") {
  ...
  async post(url2, data) {
    const response = await this.client.post(url2, data);
    return response.data;
  }
  async put(url2, data) {
    const response = await this.client.put(url2, data);
    return response.data;
  }
  async delete(url2) {
    const response = await this.client.delete(url2);
    return response.data;
  }
  async postFormData(url2, formData) {
    const response = await this.client.post(url2, formData, {
      headers: {
        "Content-Type": "multipart/form-data"
      }
    });
...
const LoginResponseSchema = objectType({
  access_token: stringType(),
  token_type: literalType("bearer")
});
objectType({
...
const ProfileSchema = objectType({
  ...
  name: stringType(),
  tone: stringType(),
  constraints: arrayType(stringType()),
  format: stringType().optional(),
  audience: stringType().optional(),
  glossary: recordType(stringType()).optional(),
  max_words: numberType().optional().default(350)
});
const RewriteOptionsSchema = objectType({
  temperature: numberType().default(0.5),
  provider_hint: stringType().default("openai")
});
const RewriteRequestSchema = objectType({
  transcript: stringType(),
  profile: ProfileSchema,
  options: RewriteOptionsSchema
});
const RewriteUsageSchema = objectType({
  stt_ms: numberType(),
  llm_ms: numberType()
});
const RewriteResponseSchema = objectType({
  draft: stringType(),
  usage: RewriteUsageSchema
});
class ApiService {
  constructor(http2, tokenStore2) {
  ...
  async login(request) {
    try {
      ...
      const response = await this.http.post("/v1/auth/login", validatedRequest);
  ...
  async transcribe(audioBlob, language) {
    try {
      ...
      const response = await this.http.postFormData("/v1/transcribe", formData);
      return response;
    } catch (error) {
  ...
  async rewrite(request) {
    try {
      ...
      const response = await this.http.post("/v1/rewrite", validatedRequest);
  ...
  handleApiError(error, defaultCode) {
    var _a, _b;
    if (error instanceof AxiosError) {
      ...
      switch (status) {
        case 400:
          return {
            code: "BAD_REQUEST",
            message: (data == null ? void 0 : data.message) || "Invalid request data",
            details: data
          };
        case 401:
          return {
        ...
        default:
          ...
          return {
            code: defaultCode,
            message: (data == null ? void 0 : data.message) || error.message || "An unexpected error occurred"
          };
      }
    }
